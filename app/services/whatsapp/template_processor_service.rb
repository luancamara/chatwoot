  ...
  def build_header_params(header_data, template)
    ...
    header_data.each do |key, value|
      ...
      if media_url_with_type?(key, header_data)
        ...
        header_params << build_text_parameter(template, key, value)
      end
    end
    header_params
  end

  # NAMED templates require parameter_name on every parameter (header included);
  # positional templates use the value alone.
  def build_text_parameter(template, key, value)
